  ...
  return (
    <div className="space-y-6">
      <div className="rounded-xl border border-slate-800 bg-slate-900 p-5">
        ...
        <h1 className="mt-1 text-xl font-bold text-slate-100">CEO Escalations</h1>
        <p className="mt-1 text-xs text-slate-400">Critical issues that have reached CEO level for strategic decision.</p>
      </div>
        ...
        {escalations.map((item) => (
          ...
          >
            ...
            <div className="mt-1 text-xs text-slate-400">
              {item.customer_name} · Escalated by {item.raised_by_role} · {formatRelativeTime(item.created_at)}
            </div>
          </Link>
        ))}
      </div>

...
export function renderDailyStatusEmailHtml(params: {
  ...
  const title = `CareYu Daily Status — ${params.period === 'morning' ? 'Morning' : 'Evening'} · ${formatSheetDate(params.date)}`;
  const headerCell =
  ...
  const groups: Array<{ person: string; rows: DailyStatusRow[] }> = [];
  for (const row of sorted) {
    const last = groups[groups.length - 1];
    if (last && last.rows[0]?.personId === row.personId) last.rows.push(row);
    else groups.push({ person: row.person, rows: [row] });
  }
  const rowsHtml = groups
    .map((group) =>
      group.rows
        ...
        <td style="${cell}${deadlineInlineStyle(deadlineTone(row.status, row.deadlineIso || row.deadline))}">${escapeHtml(row.deadline)}</td>
...
<body style="margin:0;padding:0;background:#F4F7FB;font-family:Segoe UI,Roboto,Helvetica,Arial,sans-serif;">
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#F4F7FB;padding:24px 12px;">
    <tr>
      <td align="center">
        <table role="presentation" width="100%" style="max-width:960px;background:#ffffff;border:1px solid #e2e8f0;border-radius:12px;overflow:hidden;">
          <tr>
            <td style="background:#0B1F3A;padding:18px 22px;color:#ffffff;">
              ...
              <div style="font-size:13px;color:#cbd5e1;margin-top:4px;">Hello ${escapeHtml(params.recipientName)}, this report uses the same Daily Status records as the hub.</div>
            </td>
          </tr>
          <tr>
            <td style="padding:16px 12px 24px;">
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;width:100%;">
                ...
                <tbody>
                  ${params.rows.length ? rowsHtml : empty}
                </tbody>
              </table>
            </td>
          </tr>
